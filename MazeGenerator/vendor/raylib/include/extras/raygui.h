            ...
            while (!feof(rgsFile))
            {
                switch (buffer[0])
                {
                    case 'p':
                    {
                        ...
                        int val = sscanf(buffer, "p %d %d 0x%x", &controlId, &propertyId, &propertyValue);
...
char **GuiLoadIcons(const char *fileName, bool loadIconsName)
{
    ...
    if (rgiFile != NULL)
    {
            ...
            (signature[3] == ' '))
        {
            ...
            fread(guiIcons, (size_t)iconCount*(iconSize*iconSize/32), sizeof(unsigned int), rgiFile);
        }
